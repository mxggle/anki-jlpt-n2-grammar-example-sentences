...
## ⚠️ AnkiWeb 描述 HTML 的特殊处理

文件：`ankiweb-description.html` / `ankiweb-description-minimal.html`（贴到 AnkiWeb 牌组描述字段）。

AnkiWeb 描述字段解析很脆弱，**标签之间的空白会被误判为换行/多余行**，导致排版错乱、间距过大、用户要滚动很久才能到"开始学习"。

编辑这些文件时必须遵守：

- **全部压缩成单行**，标签与标签之间不能有任何空格、缩进或换行（HTML 给机器看，不要为可读性 prettify / 分行）。
- **禁止全角空格 `　`**（U+3000）—— Anki 会把它当成换行。需要分隔符时用半角 ` | ` 或 ` / `。
- 文字内容内部的半角空格（如 emoji 与文字之间）可保留，HTML 渲染时会折叠，不影响。
- 用最小间距：`margin`/`padding` 取小值；列表标题用 `<b>` 而非 `<h3>`（`<h3>` 默认上下间距大）。

> 同样的规则也记录在 `CLAUDE.md`，两份需保持同步。
